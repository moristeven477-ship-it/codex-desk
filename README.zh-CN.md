...
默认跟随有效的 Codex CLI 配置；也可以主动选择只读、标准或 YOLO。权限由 Codex 执行。

「设置 → 通用 → 界面语言」可切换 English / 简体中文。「外观」可切换深浅色；「字体大小」可调节消息、输入框、界面与内置 CLI 的字号，设置自动保存。原生 Ubuntu Terminal 保留自己的字体设置。
...
需要 Node.js 22.16+、npm、Python 3、C 编译器（Ubuntu 的 `build-essential`），以及能正常使用的 Codex CLI。原生终端测试另需 `gnome-terminal python3-gi gir1.2-gtk-3.0 openbox xvfb dbus-x11 x11-utils xauth libxtst6`。
